...
udp_port1="5513"
udp_port2="5515"
export UUID=${UUID:-'a276f12f-bad7-478b-bc59-40b5b5b514c1'}
###############################################################
...
get_links() {
  ...
  # ISP="S${HOSTNAME:1:1}-Serv00"
  sleep 1
  # yellow "注意：v2ray或其他软件的跳过证书验证需设置为true,否则hy2或tuic节点可能不通\n"
  cat >list.txt <<EOF
vmess://$(echo "{ \"v\": \"2\", \"ps\": \"vmess-$ISP\", \"add\": \"$IP\", \"port\": \"$tcp_port\", \"id\": \"$UUID\", \"aid\": \"0\", \"scy\": \"none\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"\", \"path\": \"/vmess?ed=2048\", \"tls\": \"\", \"sni\": \"\", \"alpn\": \"\", \"fp\": \"\"}" | base64 -w0)

vmess://$(echo "{ \"v\": \"2\", \"ps\": \"tls-$ISP\", \"add\": \"www.visa.com.tw\", \"port\": \"443\", \"id\": \"$UUID\", \"aid\": \"0\", \"scy\": \"none\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/vmess?ed=2048\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"\"}" | base64 -w0)

hysteria2://$UUID@$IP:$udp_port1/?sni=www.bing.com&alpn=h3&insecure=1#Hy2-$ISP

tuic://$UUID:admin123@$IP:$udp_port2?sni=www.bing.com&congestion_control=bbr&udp_relay_mode=native&alpn=h3&allow_insecure=1#Tun-$ISP
EOF
  cat list.txt
  purple "list.txt saved successfully"
  purple "Running done!"
  sleep 3
  rm -rf web bot npm boot.log config.json 
